...
class TertiarySalesService(
    ...
    async def get_numeric_distribution(
        ...
        base_stmt = (
            ...
            .join(Pharmacy, TertiarySalesAndStock.pharmacy_id == Pharmacy.id)
            .join(Distributor, TertiarySalesAndStock.distributor_id == Distributor.id)
            .outerjoin(GeoIndicator, Pharmacy.geo_indicator_id == GeoIndicator.id)
            .join(
                total_pharmacies_cte,
                and_(
    ...
    async def get_stock_report(
        ...
        for dim in filters.group_by_dimensions:
            ...
            id_labeled = cfg["id"]
            name_min = func.min(cfg["name"].element).label(cfg["name"].key)
            case_pivot_select.extend([id_labeled, name_min])
            case_pivot_group_by.append(id_labeled.element)
            pivot_sort_map_full[dim] = name_min

        pivot_stmt = (
            select(*case_pivot_select, *period_cols)
            .select_from(TertiarySalesAndStock)
            .join(SKU, TertiarySalesAndStock.sku_id == SKU.id)
            .join(Brand, SKU.brand_id == Brand.id)
            .join(PromotionType, SKU.promotion_type_id == PromotionType.id)
        ...
        for row in rows:
            ...
            for dim in filters.group_by_dimensions:
                ...
                dim_data[f"{dim}_name"] = row[f"{dim}_name"]
            periods_data: dict = {}
            for y, m, key_str in period_keys_list:
                periods_data[key_str] = {
                    "packages": float(row[f"pkg_{y}_{m}"] or 0),
                    "amount": float(row[f"amt_{y}_{m}"] or 0),
                }
