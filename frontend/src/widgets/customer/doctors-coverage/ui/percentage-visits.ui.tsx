    ...
    ({
      ...
      const percentageQuery = useKeepQuery(
        DbQueries.GetDbItemsQuery<DoctorsCoverageRow[]>(
          ...
          {
            medical_facility_ids: isAllMedicalFacilities
              ? undefined
              : filters.medical_facility_ids,
            speciality_ids: isAllSpecialities
              ? undefined
              : filters.speciality_ids,
            product_group_ids:
              groupIds.length === groupItems.length ? undefined : groupIds,
                  ...
                  groupIds.length > 0 &&
                    groupIds.length !== groupItems.length && {
                      ...
                      getLabelFromValue(value) {
                        return (
                          groupItems.find(item => item.value === value)
                            ?.label ?? '-'
                        );
                      },
                      onDelete: () =>
                        setGroupIds(
                          groupItems.map(item => item.value).map(Number)
                        ),
                      main: {
                        onDelete: v => {
